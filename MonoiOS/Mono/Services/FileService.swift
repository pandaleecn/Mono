        ...
        do {
            ...
            for url in contents {
                if supportedExtensions.contains(url.pathExtension.lowercased()) {
                    // 先尝试从缓存获取时长
                    let cachedDuration = cacheQueue.sync { durationCache[url] }
    ...
    func loadDurations(for tracks: [AudioTrack], completion: @escaping ([AudioTrack]) -> Void) {
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            for track in tracks {
                ...
                // 检查缓存
                let cachedDuration = self.cacheQueue.sync { self.durationCache[track.url] }
                
                if let duration = cachedDuration {
                    mutableTrack.duration = duration
                } else {
                    // 获取时长并缓存
                    let duration = self.getAudioDuration(url: track.url)
                    mutableTrack.duration = duration
                    
                    self.cacheQueue.async {
                        self.durationCache[track.url] = duration
                    }
                }
    ...
    func deleteTrack(_ track: AudioTrack) -> Bool {
        do {
            ...
            cacheQueue.async { [weak self] in
                self?.durationCache.removeValue(forKey: track.url)
            }
            return true
        } catch {
